...
The goal of this excercise is to try to fit a logistic regression model on Loan data aiming to predict the probability of delinquency for each contract.
...
Later we calculated another auxiliary table that contains the proportion of each kind of transaction (k_symbol) for each account. The idea of these variable is to capture the spend pattern of each client.
...
We end up with a data set with 39 variables.
...
Alternatvely we fit a second model only with variables statistically significant p-value less than 10%.
...
We start this step by making predictions using our model on the X's variables in our Train and Test datasets.
