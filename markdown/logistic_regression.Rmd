...
Using the vanilla transaction dataset we calculate several derived variables for each account.

First we calculated an additional table with the current account balance and average account balance of each account.

Later we calculated another auxiliary table that contains the proportion of each kind of transaction for each account, the idea os these variable is to capture the spend pattern of each client.

Finnaly we combine the 682 Loan observations with client, district, credit card and the auxiliary tables we calculated early.

We end up with a data set with 51 variables. from this dataset wqe excluded 12 variables that are redundant, shows no variability for the 682 Loan contract observations or have no applicability for the excersice such as client_id, disp_id, card_id an so on.
